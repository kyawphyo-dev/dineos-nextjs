...
import { CATEGORIES, MENU_ITEMS } from "../data/mock";
...
export default function MenuScreen({ cart, onUpdateCart, onBack, onViewOrders }: Props) {
  ...
  const updateItem = (item: MenuItem, delta: number) => {
    ...
    if (!existing) {
      if (delta > 0) onUpdateCart([...cart, { ...item, qty: 1 }]);
      return;
    }
    ...
    if (newQty <= 0) {
      ...
      onUpdateCart(cart.map((c) => (c.id === item.id ? { ...c, qty: newQty } : c)));
    }
  };
            ...
            {filtered.map((item) => (
              ...
              >
                <MenuItemCard
                  item={item}
                  qty={getQty(item.id)}
                  onAdd={() => updateItem(item, 1)}
                  onRemove={() => updateItem(item, -1)}
                />
              </motion.div>
            ))}
          </AnimatePresence>
