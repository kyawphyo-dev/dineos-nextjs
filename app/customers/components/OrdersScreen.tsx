...
const STATUS_STEPS: { key: OrderStatus; label: string; time?: string }[] = [
          ...
          return (
            <div key={step.key} className="flex gap-3">
              <div className="flex flex-col items-center">
                ...
                {!isLast && (
                  <div
                    className={`w-px flex-1 my-0.5 min-h-[20px] ${
                      isDone ? "bg-clay/40" : "bg-black/10"
                    }`}
                  />
                )}
              </div>
...
export default function OrdersScreen({ cart, onBack, onAddMore }: Props) {
  const cartTotal = cart.reduce((s, i) => s + i.price * i.qty, 0);

  // Combine mock orders with any live cart items
