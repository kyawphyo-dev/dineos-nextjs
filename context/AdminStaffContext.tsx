...
import { INITIAL_STAFF } from "@/app/data/admin-mock";
import type { AdminViewRole, StaffMember, StaffRole } from "@/app/types/admin";

interface StaffContextValue {
  staff: StaffMember[];
  addStaff: (member: Omit<StaffMember, "id">, actingRole: AdminViewRole) => { ok: boolean; reason?: string };
  updateStaffRole: (id: string, role: StaffRole, actingRole: AdminViewRole) => { ok: boolean; reason?: string };
  removeStaff: (id: string, actingRole: AdminViewRole) => { ok: boolean; reason?: string };
}
...
export function StaffProvider({ children }: { children: ReactNode }) {
  ...
  const addStaff = (member: Omit<StaffMember, "id">, actingRole: AdminViewRole) => {
  ...
  const updateStaffRole = (id: string, role: StaffRole, actingRole: AdminViewRole) => {
  ...
  const removeStaff = (id: string, actingRole: AdminViewRole) => {
    const target = staff.find((s) => s.id === id);
    if (!target) return { ok: false, reason: "Staff member not found." };
