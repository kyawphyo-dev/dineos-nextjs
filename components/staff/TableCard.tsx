...
const STATUS_STYLES: Record<FrontTable["status"], { bg: string; text: string; border: string }> = {
  ...
  need_attention: { bg: "bg-rose-light", text: "text-rose", border: "border-rose/30 border-2" },
  request_bill: { bg: "bg-purple-100", text: "text-purple-700", border: "border-purple-300 border-2" },
...
export default function TableCard({ table, selected, onClick }: Props) {
  ...
  return (
    ...
    >
      {(table.status === "need_attention" || table.status === "request_bill") && (
        <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full animate-pulse"
          style={{ backgroundColor: table.status === "need_attention" ? "#f43f5e" : "#7c3aed" }}
        />
      )}
